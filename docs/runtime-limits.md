...
A request occupies one pending slot after it has been admitted and before it returns, fails, or is cancelled. When the limit is reached, Rivet returns an Error frame for the new request instead of creating another request custodian and Racket thread.
