...
Native clients preserve the same ownership rule when generating IDs. Windows and macOS allocate only non-zero UInt64 IDs, skip any ID that is still present in their pending table, and perform ID selection plus pending registration under the same lock. UInt64 wraparound therefore resumes at 1 without overwriting an older request. On Swift, Task cancellation is latched until the Request frame has been written successfully; Cancel can never overtake its Request on the wire, and cancellation that wins before registration emits neither frame.
