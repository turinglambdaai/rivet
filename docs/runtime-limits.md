...
Native clients preserve the same ownership rule when generating IDs. Windows and macOS allocate only non-zero UInt64 IDs, skip any ID that is still present in their pending table, and perform ID selection plus pending registration under the same lock. UInt64 wraparound therefore resumes at 1 without overwriting an older request. Native cancellation is latched until the corresponding Request frame has been written successfully, so Cancel can never overtake Request on the wire. Swift Task cancellation that wins before registration emits neither frame and its internal cancellation state emits at most one Cancel; Windows likewise collapses repeated `cancel(id)` calls for the same pending Request to one Cancel frame.
