...
(define (all-types rpcs states)
  (remove-duplicates
   (append
    ...
    (append* (for/list ([state (in-list states)])
...
(define (swift-type type)
  (match type
    ['String "String"] ['Int64 "Int64"] ['Bool "Bool"] ['Bytes "Data"]
    ['Void "Void"] ['Any "RivetValue"]
...
(define (cpp-type type)
  (match type
    ['String "std::string"] ['Int64 "std::int64_t"] ['Bool "bool"]
    ['Bytes "rivet::Bytes"] ['Void "void"] ['Any "rivet::Value"]
...
(define (swift-rpc-method info)
  ...
  (define params
    (string-join (for/list ([n (in-list names)] [t (in-list types)])
                   (format "~a: ~a" n (swift-type t))) ", "))
  (define encoded
    (string-join (for/list ([n (in-list names)] [t (in-list types)])
                   (format "encode_~a(~a)" (type-key t) n)) ", "))
  (format "    public func ~a(~a) async throws -> ~a {\n        let result = try await client.call(~s, arguments: [~a])\n        return try decode_~a(result)\n    }\n"
...
(define (swift-state-methods state)
  ...
  (format "    public func get~a() async throws -> ~a {\n        return try decode_~a(await client.getState(~s))\n    }\n    @discardableResult public func set~a(_ value: ~a) async throws -> ~a {\n        return try decode_~a(await client.setState(~s, value: encode_~a(value)))\n    }\n"
          suffix (swift-type type) (type-key type) raw-name
          suffix (swift-type type) (swift-type type) (type-key type) raw-name (type-key type)))

(define (generate-swift rpcs states module-name entry-name)
  (define types (all-types rpcs states))
  (string-append
...
(define (cpp-future result-type raw-expression)
  ...
  (format "auto raw = ~a; return std::async(std::launch::deferred, [raw = std::move(raw)]() mutable -> ~a { ~a });"
...
(define (cpp-rpc-method info)
  ...
  (define params
    (string-join (for/list ([n (in-list names)] [t (in-list types)])
                   (format "~a ~a" (cpp-type t) n)) ", "))
  (define encoded
    (string-join (for/list ([n (in-list names)] [t (in-list types)])
                   (format "detail::encode_~a(~a)" (type-key t) n)) ", "))
  (format "  std::future<~a> ~a(~a) { ~a }\n"
          (cpp-type result-type) (cpp-id (schema-rpc-name info)) params
          (cpp-future result-type
                      (format "backend_.call(~s, rivet::Value::List{~a})"
                              (symbol->string (schema-rpc-name info)) encoded))))

(define (cpp-state-methods state)
  (define name (cpp-id (schema-state-name state)))
  (define raw-name (symbol->string (schema-state-name state)))
  (define type (schema-state-type state))
  (format "  std::future<~a> get_~a() { ~a }\n  std::future<~a> set_~a(~a value) { ~a }\n"
          (cpp-type type) name
          (cpp-future type (format "backend_.get_state(~s)" raw-name))
          (cpp-type type) name (cpp-type type)
          (cpp-future type (format "backend_.set_state(~s, detail::encode_~a(value))" raw-name (type-key type)))))

(define (generate-cpp rpcs states module-name entry-name)
  (define types (all-types rpcs states))
  (string-append
   "// Generated by Rivet. Do not edit by hand.\n#pragma once\n\n#include <cstdint>\n#include <future>\n#include <optional>\n#include <stdexcept>\n#include <string>\n#include <utility>\n#include <vector>\n\n#include \"backend.hpp\"\n\nnamespace rivet_app {\n"
   (format "inline constexpr char kModuleName[] = ~s;\ninline constexpr char kEntryName[] = ~s;\n\n" module-name entry-name)
   "namespace detail {\n" (apply string-append (map cpp-encoder types)) "\n"
   (apply string-append (map cpp-decoder types)) "}  // namespace detail\n\n"
...
(define (generate-clients! project)
  ...
  (define-values (rpcs states) (load-schema backend))
  ...
  (write-generated!
   ...
   (generate-swift rpcs states module-name entry-name))
  (write-generated!
   (project-path project "windows" "GeneratedBackend.hpp")
   (generate-cpp rpcs states module-name entry-name))
  (values rpcs states))
