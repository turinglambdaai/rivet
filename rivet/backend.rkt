...
(define (serve in out
  ...
  (define (start-request! f)
    ...
    ;; A duplicate Request cannot be addressed independently because its id is
    ;; already owned by the original request. Emitting an Error with that same
    ;; id would make native clients consume the Error as the original request's
    ;; terminal result. Drop the duplicate before decoding its payload so one id
    ;; can produce at most one terminal frame.
    (unless (pending-id? id)
      ;; Bad application requests must fail that request rather than tear down
      ;; the entire embedded runtime. The generated client should not produce
      ;; these frames, but keeping the server alive makes failures local.
      (with-handlers ((exn:fail?
                       (lambda (e)
        ...
        (cond
          ((eq? admission 'duplicate)
           ;; The reader is single-threaded, so the early pending-id? check
           ;; normally makes this unreachable. Keep it silent as a defensive
           ;; fallback rather than violating the one-terminal-per-id invariant.
